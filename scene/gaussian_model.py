...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # todo new 2. 增强的2D高斯初始化
        # 原初始化方法只考虑点密度，忽略局部几何特征
        # 基于局部曲率的初始化能更好适应复杂表面
        def improved_initialization(pcd: BasicPointCloud, spatial_lr_scale: float):
            """改进的2D高斯初始化，考虑局部几何"""
            fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
            ...
            if len(points_np) > 100:  # 只有点足够多时才计算局部几何
                nbrs = NearestNeighbors(n_neighbors=10).fit(points_np)
                distances, indices = nbrs.kneighbors(points_np)

                # 估计局部曲率作为初始缩放
                local_variation = np.mean(distances, axis=1)
                scales = torch.log(torch.from_numpy(local_variation).float().cuda())[..., None].repeat(1, 2)
            else:
                # 回退到原始方法
                dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
                scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 2)
            ...
            return scales
        # 替换原有的缩放初始化
        scales = improved_initialization(pcd, spatial_lr_scale)
